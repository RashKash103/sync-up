...
def normalize_compatible_packages(cp):
    """Normalize compatiblePackages to the list-of-objects shape used below.

    Newer morphe-patches-library versions emit:
        [{ "packageName": ..., "name": ..., "targets": [{ "version": ... }] }]
    Older versions (including the one this repo is pinned to) emit:
        ...
        return [
            {
                "packageName": pkg,
                "name": pkg,
                "targets": [{"version": v} for v in (versions or [])],
            }
            for pkg, versions in cp.items()
        ]
    return cp


for patch in data["patches"]:
    cp = normalize_compatible_packages(patch.get("compatiblePackages"))
    if not cp:
