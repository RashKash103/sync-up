...
# ///
"""
Rebuilds the patch bundle and runs the morphe-desktop patcher against all APKs in test/.

Patch options are read from test/config.json when that file exists; see the Morphe
documentation for its format. Without it the patches run with their default options,
which means the Spoof client patch will fail on its required client-id option.

Usage: uv run test/test_all_apks.py
"""
...
# morphe-cli was renamed to morphe-desktop; its fat jar still provides the patch CLI.
GITHUB_RELEASES_URL = (
...
def download_latest_cli_jar() -> Path:
    """
    Download the latest morphe-desktop fat jar from GitHub releases (stable or pre-release).

    Returns a cached copy if the same version was already downloaded.
    """
    print("Fetching latest morphe-desktop release from GitHub ...")
...
def patch_apk(jar: Path, patches: Path, apk: Path) -> subprocess.CompletedProcess:
    """Run the Morphe patcher on a single APK.

    Patch options come from test/config.json when present; otherwise the patches run
    with their defaults.
    """
    cmd = [
        "java", "-jar", str(jar),
    ...
    options_file = TEST_DIR / "config.json"
    if options_file.exists():
        cmd.append(f"--options-file={options_file}")
    else:
        print(f"  No {options_file} found; patching with default patch options.")
    cmd.append(str(apk))
